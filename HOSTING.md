...
The initial Sites host automatically records traffic analytics. That conflicts with the brief's ban on analytics/tracking, so this package omits the Sites manifest. Do not restore that host unless the restriction can be met. Before corrective submission, the published host must pass fresh anonymous browser checks for cookies, learner-state transmission, injected scripts and exact file contents. These public checks are pending in this preparation snapshot. Hosting services may retain ordinary server logs; this project does not enable analytics or claim to control provider infrastructure.
